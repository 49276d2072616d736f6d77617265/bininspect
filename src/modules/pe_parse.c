...
static int fail(PeInfo *out, int rc) {
    if (out) { out->present = 0; out->parse_rc = rc; }
    return rc;
}
...
int pe_parse_minimal(const uint8_t *d, uint64_t n, PeInfo *out) {
    ...
    if (!d || n < 0x40) return fail(out, 1);

    // "MZ"
    if (d[0] != 'M' || d[1] != 'Z') return fail(out, 2);

    uint32_t pe_off = read_u32_le(d + 0x3C);
    if ((uint64_t)pe_off + 4 + 20 > n) return fail(out, 3);

    // "PE\0\0"
    if (!(d[pe_off]=='P' && d[pe_off+1]=='E' && d[pe_off+2]==0 && d[pe_off+3]==0))
        return fail(out, 4);
    ...
    if ((uint64_t)(opt - d) + opt_size > n) return fail(out, 5);
